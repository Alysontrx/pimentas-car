...
export default function Estoque() {
  ...
  const filteredAndSortedVehicles = useMemo(() => {
    let result = mockVehicles.filter(car => {
      ...
      // Estilo não existe no mockVehicles atualmente, então ignoramos ou filtramos se existir
    ...
    return result;
  }, [search, brand, model, version, yearMin, yearMax, priceMin, priceMax, mileageMin, mileageMax, transmission, condition, colors, sortOption]);

  // As marcas disponíveis (baseadas apenas nos outros filtros, para não sumirem as opções quando a própria marca for selecionada)
  const availableBrands = useMemo(() => {
    return [...new Set(mockVehicles.filter(car => {
      if (search && !car.model.toLowerCase().includes(search.toLowerCase()) && !car.brand.toLowerCase().includes(search.toLowerCase())) return false;
      if (model && !car.model.toLowerCase().includes(model.toLowerCase())) return false;
      if (version && !car.version.toLowerCase().includes(version.toLowerCase())) return false;
  ...
  }, [search, model, version, yearMin, yearMax, priceMin, priceMax, mileageMin, mileageMax, transmission, condition, colors]);

  // === Componente do Conteúdo do Filtro (usado no Desktop e Mobile) ===
  const filterContentJsx = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
          {['Branco', 'Prata', 'Preto', 'Cinza', 'Azul'].map(c => (
